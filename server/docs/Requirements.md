...
- **Attributes:**
    ...
    - **Income Margin (30%):** Based on the total cost calculate the % income margin (totalCost * 0.3) (read-only autogenerated)
    - **IVA: 13%:** IVA tax amount (read-only auto-generated) add this percentage to config table
    - **Service Tax 10%:** Service tax amount (read-only auto-generated) add this percentage to config table
    - **Price:** Total price with good margin. These calculation might get decimals, we can round it up to next 100.  
  
- **Business Logic:**
  - Multiple ingredients can be part of one recipe
  - Total cost automatically calculated based on current ingredient prices, income margins (config) and taxes.
  - Used for pricing analysis and profit margin calculations
